...
def process_company_files(company_type, api_key):
    ...
    Args:
        ...
        api_key: OpenAI API key
    """
    # Updated base directory path - where the reports are actually saved
    base_dir = r"C:\Users\Akshila.Anurangi\financial-data-extraction-pipeline\financial_data_extractor\reports"
    
    # Output directory for CSV files
    output_dir = r"C:\Users\Akshila.Anurangi\financial-data-extraction-pipeline\dataset_creator\data"
    
    # Create output directory if it doesn't exist
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    # Updated company directory pattern with the .N0000 suffix
    company_dir_pattern = f"{company_type}.N0000"
    directory = os.path.join(base_dir, company_dir_pattern)
    
    # Output path for CSV
    output_csv = os.path.join(output_dir, f"{company_type.lower()}_financial_data_openai.csv")
    
    # Check if directory exists
    if not os.path.exists(directory):
        print(f"Directory not found: {directory}")
        # Try without .N0000 suffix as fallback
        directory = os.path.join(base_dir, company_type)
        if not os.path.exists(directory):
            print(f"Fallback directory not found either: {directory}")
            return
        print(f"Using fallback directory: {directory}")
